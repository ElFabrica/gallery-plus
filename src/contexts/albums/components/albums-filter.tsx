            ...
            <div className="flex gap-3">
                {
                    !loading ?
                        <>
                            ...
                            {albums.map(album =>
                                <Button key={album.id} size="sm" variant="ghost" className="cursor-pointer">
                                    {album.title}
                                </Button>
                            )}
                        </>
                        : Array.from({ length: 5 }).map((_, index) => <Skeleton className="w-28 h-7" key={`albun-button-loading-${index}`} />)
                }
            </div>
        </div>
    )
